...
class Encoder(nn.Module):
    ...
    def __init__(self, input_dim, encoder_hidden_dim, 
                 encoder_lockdrop=None, encoder_batchnorm=False, encoder_dropout=0.1, 
...
class Decoder(nn.Module):
    ...
    '''
    def __init__(self, vocab_size, decoder_hidden_dim, embed_dim, device, 
                 decoder_dropout=None, attn_dropout=None, key_value_size=128):
        ...
        self.lstm3 = nn.LSTMCell(decoder_hidden_dim, key_value_size)  # input=hidden1
        if decoder_dropout:
    ...
    def forward(self, key, value, encoder_len, y=None, mode='train', epoch=None):
        ...
        if epoch:
            # Decrease teacher forcing rate 0.1 every 5 epochs until 0.2
            teacher_forcing_rate = max(1 - (epoch // 5) * 0.1, 0.2)
        else:
        ...
        for i in range(max_len):
            ...
            hidden2 = hidden_states[1][0]
            if self.dropout:
                hidden2 = self.dropout(hidden2)
            hidden_states[2] = self.lstm3(hidden2, hidden_states[2])
...
class Las12(nn.Module):
    def __init__(self, input_dim, vocab_size):
        super(Las12,self).__init__()

        # Model parameters
        encoder_hidden_dim=256
        encoder_lockdrop = 0.3
        ...
        self.encoder = Encoder(input_dim, encoder_hidden_dim, 
                               encoder_lockdrop, encoder_batchnorm, encoder_dropout, 
                               trunc, key_value_size)
        self.decoder = Decoder(vocab_size, decoder_hidden_dim, embed_dim, device,
                               decoder_dropout, attn_dropout, key_value_size)
...
class Las13(nn.Module):
    def __init__(self, input_dim, vocab_size):
        super(Las13,self).__init__()
...
class Las14(nn.Module):
    def __init__(self, input_dim, vocab_size):
        super(Las14,self).__init__()
